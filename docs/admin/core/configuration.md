...
#
# Whether the roomserver is running embedded in the controller process (internal) or as an external service.
kind = "external"

# The URL the controller uses for requests to the roomserver.
# Only relevant when `roomserver.kind` is `external`.
...
# Only relevant when `roomserver.kind` is `internal`.
#public_url = "http://localhost:11333"

# Conference settings are optional and only relevant when `roomserver.kind` is `internal`.
# [roomserver.conference]
...
# Defaults are optional and only relevant when `roomserver.kind` is `internal`.
# [roomserver.defaults]
...
# Reports settings are optional and only relevant when `roomserver.kind` is `internal`.
#[roomserver.reports.typst]
