...
impl OpenTalkAuthorizerBackend {
    /// Authorize access to the [`UserFind`] resource.
    ///
    /// # Access Control List
    ///
    /// Access rights for the [`UserFind`] resource (`GET` on
    /// `/users/find`). The endpoint is the user-search lookup used
    /// by the frontend's invite picker. There is no write surface,
    /// so the matrix is `User: Read` rather than `User: rw` — any
    /// registered user may read, all write methods are denied, and
    /// invite codes have no access at all.
    ///
    /// | Subject     | Access |
    /// | ----------- | ------ |
    /// | **User**    | r-     |
    /// | **Guest**   | --     |
    ///
    /// [`UserFind`]: opentalk_controller_api_authorization::authorization::Resource::UserFind
    pub(crate) fn authorize_user_find(
        &self,
